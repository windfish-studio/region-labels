    ...
    createGroupings(geojson) {
        ...
        const aggregateFeature = (_f) => {
            ...
            if(this.opts.dbscanClustering){
                ...
                const pointGrid = generatePointGrid(this.bbox.absolute, this.opts.dbscanPointDensity, shapes);
                const pointShapeIdx = {};

                let dbscanRadius = this.bbox.absolute.width / this.opts.dbscanPointDensity;
                dbscanRadius += (dbscanRadius / 10);

                const points = pointGrid.map(_po => {return _po.point});
                pointGrid.forEach((_po, _pi) => {
                    pointShapeIdx[_pi] = _po.shape;
                });
                const groupings = dbscan.run(points, dbscanRadius, 2);

                groupings.forEach(_g => {
                    const groupShapes = [];
                    _g.forEach(_i => {
                        const groupedShape = pointShapeIdx[_i];
                        if(groupShapes.indexOf(groupedShape) == -1){
                            groupShapes.push(pointShapeIdx[_i]);
                        }
                    });

                    group = createGroup();
                    group.label = label;
                    group.shapes = groupShapes.map((verts) => {
                        return {
                            excluded: excluded,
                            vertices: verts
                        };
