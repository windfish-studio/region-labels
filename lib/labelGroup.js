...
import {arcSolver} from "./util/parabolicArclengthSolver";
import {generatePointGrid, pointInside} from "./util/generatePointGrid";
...
export class LabelGroup {
    ...
    getExtremePoint(direction){
        ...
        range(0, segments).forEach((i) => {
            ...
            const match = pointInside(thisPoint.absolute, this.polygons);
    ...
    calculateLetters() {
        ...
        const linearDataXY = generatePointGrid(this.bbox, this.pointDensity, this.polygons).map(_p => {return _p.point});
        const linearDataYX = [];
