...
function rayFromHitPoints(hitPoints){
    ...
    var min =minBy(hitPoints, function(hp){
        return hp.point[axis]
    });
    var max =maxBy(hitPoints, function(hp){
        return hp.point[axis]
    });
    return [min, max];
}
...
export function RegionLabel(geojson, opts) {
    ...
    function centerVertices(canvas) {
        ...
        var regenerateBbox = function () {
            totalBbox = RegionLabel.generateBoundingBox(groupData.map((l_group) => {
    ...
    function LabelGroup(data){
        ...
        var regX, regY, lines, spline, relativeSplineEquation, landArea, fontSize, reverseLetters, textWidth, firstPoint, lastPoint,
            ...
            polygons = data.shapes.map((p_obj) => { return p_obj.vertices });
        ...
        function groupSegments(segments){
            ...
            segmentGroups.forEach((group) => {
                ...
                const ray = rayFromHitPoints(points);
                if (ray) {
                    rays.push(ray);
                }
            });
        ...
        function computeRay(start, end){
            ...
            var sortedHitPoints = sortBy(hitPoints, [function(hp){return(hp.point[axis])}]);
            ...
            longestRay = maxBy(rays, function(p){
        ...
        function calculateLetters() {
            ...
            range(bbox.minX, bbox.maxX + 1, (bbox.width + 1) / bboxDivisions).forEach( (x) => {
                range(bbox.minY, bbox.maxY + 1, (bbox.height + 1) / bboxDivisions).forEach((y) => {
                    var p = [x,y];
                    if(pointInside(p))
                        linearDataXY.push(p);
                });
            ...
            longestRay = maxBy(validRays, function(ray){
        ...
        this.debugDraw = function (options) {
            ...
            var points = ['firstPoint', 'lastPoint'].map((_k) => {
                ...
                return p;
            }).concat(relativeMidPoints.map((p) => {
...
RegionLabel.generateBoundingBox = function (allPoints) {
    ...
    var bbox = {
        minX: minBy(_pts, function (vtx) {
            return vtx[0]
        })[0],
        minY: minBy(_pts, function (vtx) {
            return vtx[1]
        })[1],
        maxX: maxBy(_pts, function (vtx) {
            return vtx[0]
        })[0],
        maxY: maxBy(_pts, function (vtx) {
            return vtx[1]
        })[1],
        within: function (p) {
