...
function rayFromHitPoints(hitPoints){
    ...
    var min =_.minBy(hitPoints, function(hp){return hp.point[axis]});
    var max =_.maxBy(hitPoints, function(hp){return hp.point[axis]});
    return [min, max];
}
...
function RegionLabel(geojson, opts) {
    ...
    var regX, regY, lines, spline, relativeSplineEquation, landArea, fontSize, reverseLetters, textWidth, firstPoint, lastPoint,
        letterPoints, distFromLast, labelStartX, labelStartY, rayStart, rayEnd, rayCenter, rayLength, bbox,
        ...
        shapesArea = 0,
        letters = opts.label.split(""),
        world = new p2.World(),
        ctx = world_canvas.getContext('2d');


    var polygons = parseGeoJSON(geojson);
    ...
    function groupSegments(segments){
        ...
        _.each(segments, function (seg) {
            total_length += getDist(seg[0].point, seg[1].point);
        });
        ...
        var addSegments = function(){
            ...
            firstLength = getDist(first[0].point, first[1].point);
            nextLength = getDist(next[0].point, next[1].point);
            dist = getDist(first[1].point, next[0].point);

            var excluded = first[1].body.excluded || next[0].body.excluded;

            if (excluded || dist > _.max([firstLength, nextLength]) * (1/landArea)){
                segmentGroups.push(group);
                group = [];
                addSegments();
            } else {
        ...
        else return groupSegments(rays);
    }
    ...
    function getTextWidth(font){
        ...
        return ctx.measureText(opts.label).width;
    }
    ...
    function getExtremePoint(direction){
        ...
        while(!point || !lastPoint){
            var p = getPoint(i);
            var match = pointInside(p.absolute);
        ...
        var p = getHitPoints(lastPoint.absolute, point.absolute)[0];
        var t = translate(p.point, rayCenter, longestLineSlope);
    ...
    function calculateLetters() {
        _.each(polygons, function(shapes){
            _.each(shapes, function (shape, idx) {
                var body = new p2.Body();
                if(idx == 0){
                    var convex = new p2.Convex({ vertices: shape });
                    shapesArea += calcPolygonArea(shape);
                }else{
                    //add the shape's 'holes'.
                    var convex = new p2.Convex({ vertices: _.reverse(_.clone(shape)) });
                    shapesArea -= calcPolygonArea(shape);
        ...
        distFromLast -= ctx.measureText(opts.label[opts.label.length - 1]).width;
    ...
    function centerVertices(canvas) {
        ...
        if(wrapLongitude){
            doWrapLongitude();
            bbox = RegionLabel.generateBoundingBox(polygons);
        }
    ...
    function parseGeoJSON() {
        ...
        var _polygons = [];
        //parse GeoJSON
        var aggregateFeature = function(_f){
            switch(_f.geometry.type){
                case 'MultiPolygon':
                    _polygons = _polygons.concat(_f.geometry.coordinates);
                    break;
                case 'Polygon':
                    _polygons.push(_f.geometry.coordinates);
                    break;
                default:
                    throw 'geometry type '+_f.geometry.type+' not supported';
                    break;
            }
        };
        ...
        if(_g.type && _g.type == 'FeatureCollection'){
            _.each(_g.features, function (feat) {
                aggregateFeature(feat);
            });
        }else{
            aggregateFeature(_g); //geojson is a feature;
        }
...
RegionLabel.generateBoundingBox = function (polygons) {
    //double-flatten polygons to get a single list of verts
    var allPoints = Array.prototype.concat.apply([], Array.prototype.concat.apply([], polygons));
