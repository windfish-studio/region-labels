    ...
    function centerVertices(canvas) {
        ...
        if(opts.wrapLongitude){
            _.each(groupData, function(l_group){
                RegionLabel.doWrapLongitude(l_group.shapes, opts.wrapLongitudeTolerance);
            });
        }
...
RegionLabel.doWrapLongitude = function (features, tolerance) {
    ...
    _.each(features, function (feat, f_idx) {
        allShapesBbox.push([]);
        _.each(feat.vertices, function (shape, s_idx) {
            var newBbox = RegionLabel.generateBoundingBox([[shape]]);
            allShapesBbox[f_idx].push(newBbox);
            if(!largestBbox){
                largestBbox = newBbox;
    ...
    _.each(features, function (feat, f_idx) {
        ...
        var newPoly = [];
        _.each(feat.vertices, function (shape, s_idx) {
            var shapeBbox = allShapesBbox[f_idx][s_idx];
            var newShape = [];
