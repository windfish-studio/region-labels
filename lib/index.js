...
function main(shapes, text, ctx) {
    ...
    var fontSize = 100;//parseInt(curveLength / text.length);
    ...
    if (longestLineSlope >=0){
        ...
        if (splinePointSets[0].absolute[1] < splinePointSets[splinePointSets.length - 1].absolute[1])
            splinePointSets.reverse();
    }

    var letterData = [];

    var placeLetters = function(){
        ...
        var nextPointIdx = (startPointIdx > 0)? startPointIdx : 0;

        _.each(letters, function(letter) {
            var letterWidth = ctx.measureText(letter).width + letterPadding;
            lettersWithPoints.push({letter: letter, point: splinePointSets[nextPointIdx]});
            nextPointIdx += Math.round(pointsPerPixel * letterWidth);
        });
        var notFitting = _.some(lettersWithPoints, ['point', undefined]);
        if (notFitting){
            fontSize *= 0.75;
            if (fontSize < 1)
    ...
    var letterPoints = placeLetters();

    _.each(letterPoints, function(obj) {
        if (!obj.point) return;
        var slope = 2 * relativeSplineEquation[2] * obj.point.relative[0] + relativeSplineEquation[1];
        var angle = Math.atan(slope);
        if (longestLineSlope < 0)
            angle *= -1;
        angle += longestLineAngle;

        letterData.push({letter: obj.letter, angle: angle, point: obj.point.absolute, font: ctx.font});
    });
    return letterData;
}
