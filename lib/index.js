...
function isValidLine (bbox, line, flipped){
    var idx, min, max;
    if (!flipped){
...
function regressionLines (bbox, m, b, flipped){
    var slope = m,
        yIntercept = b,
    ...
    var lineLoop = function(){
        ...
        if (isValidLine(bbox, line, flipped)){
            lines.push(line);
            lineLoop();
        }
    };
...
function addPerpendicularRelativeMidPoint(world, line, offset, center, points){
    var ray = castRay(world, line);
...
function main(shapes, ctx) {
    var regX, regY, lines, splineEquation, longestLineSlope, longestRay, rayStart, rayEnd, rayCenter, rayLength, numPerpSegments, raySegment, rayHalf,
        rayOffsets = [],
        validRays = [],
        relativeMidPoints = [],
        untranslatedPointsX = [],
    ...
    switch (longestLineSlope){
        case 0:
            _.each(rayOffsets, function(offset){
                ...
                addPerpendicularRelativeMidPoint(world, line, offset, rayCenter, relativeMidPoints);
        ...
        default:
            ...
            _.each(rayOffsets, function(offset){
                ...
                if(!isValidLine(bbox, line, true))
                    line = newLine(bbox, perpSlope, intercept, true);
                perpendicularLines.push(line);
                addPerpendicularRelativeMidPoint(world, line, offset, rayCenter, relativeMidPoints);
            });
    }
