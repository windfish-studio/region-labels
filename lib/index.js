...
function RegionLabel(shapes, text) {
    ...
    this.untranslate = function(point, center, slope){
    ...
    function getExtremePoint(i){
        var point, raySegmentIdx = 0;
        while (!point){
            var x = (i === 'first')? -rayHalf + raySegmentIdx * raySegment : rayHalf - raySegmentIdx * raySegment;
            var y = relativeSplineEquation[2] * Math.pow(x, 2) + relativeSplineEquation[1] * x + relativeSplineEquation[0];
            var p = scope.untranslate([x, y], rayCenter, longestLineSlope);
            _.each(shapes, function (shape) {
                if (pip(p, shape)) {
                    point = {absolute: p, relative: [x, y]};
                    return false;
                }
            });
            raySegmentIdx++;
    ...
    if (firstPoint.absolute[0] > lastPoint.absolute[0] || (longestLineSlope < 0 && firstPoint.absolute[1] < lastPoint.absolute[1])){
        reverseLetters = true;
    ...
    _.each(letterPoints, function(obj) {
        ...
        var absolutePoint = scope.untranslate([x,y], rayCenter, longestLineSlope);
