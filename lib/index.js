...
function RegionLabel(geojson, opts) {
    ...
    function parseGeoJSON() {
        ...
        var aggregateFeature = function(_f, group){
            ...
            switch(_f.geometry.type){
                case 'MultiPolygon':
                    _.each(_f.geometry.coordinates, function (verts) {
                        group.shapes.push({
                            excluded: excluded,
                            vertices: verts
                        });
                    ...
                    break;
                case 'Polygon':
                    var verts = _f.geometry.coordinates;
                    group.shapes.push({
                        excluded: excluded,
                        vertices: verts
                    });
    ...
    function centerVertices(canvas) {
        ...
        function doWrapLongitude(){
            ...
            var testLongitude = function(x1, x2){
                return (Math.abs(x2 - x1) < opts.wrapLongitudeTolerance);
            };
    ...
    function LabelGroup(data){
        ...
        var regX, regY, lines, spline, relativeSplineEquation, landArea, fontSize, reverseLetters, textWidth, firstPoint, lastPoint,
            letterPoints, distFromLast, labelStartX, labelStartY, rayStart, rayEnd, rayCenter, rayLength,
