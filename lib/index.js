...
export class RegionLabel {
    ...
    constructor(geojson, opts) {
        ...
        //set default options
        this.opts.canvasDims = opts.canvasDims || [256, 256];
        this.opts.label = opts.label || function (_props) { return _props.NAME; };
        this.opts.excludeFeatures = opts.excludeFeatures || function () { return false; };
        this.opts.excludeHoles = (opts.excludeHoles === undefined)? true : opts.excludeHoles;
        this.opts.wrapLongitude = (opts.wrapLongitude === undefined)? true : opts.wrapLongitude;
        this.opts.wrapLongitudeTolerance = (opts.wrapLongitudeTolerance === undefined)? 180 : opts.wrapLongitudeTolerance;

        this.totalBbox = null;
        this.margin = opts.margin || 0;
        this.world_canvas = opts.canvas || spawnCanvas(opts.canvasDims);
        this.groupData = [];
        this.labelGroups = [];

        this.parseGeoJSON(geojson);
        this.centerVertices(this.world_canvas);
    ...
    parseGeoJSON(geojson) {
        ...
        var singleGroup = (typeof this.opts.label != 'function');
    ...
    calculateGroupLabels(){
        this.groupData.forEach((g_data) => {
            this.labelGroups.push(new LabelGroup(g_data, this.totalBbox, this.world_canvas));
        });
    };

    drawEachGroup(todo, args) {
        const canvas = spawnCanvas([this.world_canvas.width, this.world_canvas.height]);
