...
function newLine(bbox, slope, yIntercept, flipped) {
    ...
    line.flipped = (flipped)? true : false;
    return line;
}

function lineLoop(lines, offset, bbox, slope, yInt, flipped){
    yInt += offset;
    var line = newLine(bbox, slope, yInt, flipped);
    if (pip(line.start, bbox.points) || pip(line.end, bbox.points)){
        lines.push(line);
        lineLoop(lines, offset, bbox, slope, yInt, flipped);
    }
}
...
function main(shapes, ctx) {
    var regX, regY, lines, splineEquation, longestLineSlope, longestLineIntercept,
        longestLinePerpIntercept, longestRay, rayStart, rayEnd, rayCenter, rayLength,
        rayFlipped, numPerpSegments, raySegment, rayHalf,
        rayOffsets = [],
        validRays = [],
        relativeMidPoints = [],
        untranslatedPointsX = [],
    ...
    lines = regressionLines(bbox, regX.equation[0], regX.equation[1]).concat( regressionLines(bbox, regY.equation[0], regY.equation[1], true) );
    ...
    var longestLinePSlope = -1 / longestLineSlope;
    var longestLinePIntercept = rayCenter[1] - (-1/longestLineSlope) * rayCenter[0];
    ...
    var addPerpendicularRelativeMidPoint = function(start, end, offset){
        ...
        var doInvert = function(_slope, _intercept, _start, _end){
            return (_slope == -Infinity && _end[0] - _start[0] < 0) || //special case for Infinity slope
                (midPoint[1] < ((_slope * midPoint[0]) + _intercept)); //linear inequality y < mx + b, plugging in midpoint
        }

        if(doInvert(longestLinePSlope, longestLinePIntercept, offset, rayCenter)){
            relativeX = relativeX * -1; //assign directionality
        }

        if( doInvert(longestLineSlope, longestLineIntercept, midPoint, offset) ){
    ...
    _.each(rayOffsets, function(offset) {
        ...
        switch (longestLineSlope) {
            ...
            default:
                ...
                var line = newLine(bbox, perpSlope, intercept);
                addPerpendicularRelativeMidPoint(line.start, line.end, offset);
        }
    });
    var spline = regression('polynomial', relativeMidPoints, 2);
    ...
    //
    // _.each(spline.points, function(p){
    //     var point = untranslate([p[0],p[1]], rayCenter, longestLineSlope);
